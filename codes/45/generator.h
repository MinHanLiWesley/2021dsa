...
void _rnd_genarray(int n, int *a, int l, int r, unsigned char *v, int flush)
{
    ...
    {
        if (flush)
            memset(v + l, 0, (r - l + 1) * sizeof(unsigned char));
        for (int i = 0; i < n; i++)
        {
            do
            {
...
void _generator_getdata(int *rn, int **rp, int **rq, int **rr)
{
    ...
    int n = _generator_n;
    if (_generator_task == 2 || _generator_task == 3)
        _rnd_genarray(n, p, -_generator_v, _generator_v - 1, vis, 1);
    else
        _rnd_genarray(n, p, -_generator_v, _generator_v - 1, NULL, 0);
